...
public class StatisticsTool {
	...
	public synchronized void printDynamic(String foo) {
		...
		if (dyn_method_count.get(threadId) == 0) {
			return;
		}

		float instr_per_bb = (float) dyn_instr_count.get(threadId) / (float) dyn_bb_count.get(threadId);
		float instr_per_method = (float) dyn_instr_count.get(threadId) / (float) dyn_method_count.get(threadId);
		float bb_per_method = (float) dyn_bb_count.get(threadId) / (float) dyn_method_count.get(threadId);
